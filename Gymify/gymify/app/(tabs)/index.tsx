  ...
  useEffect(() => {
    async function getUser() {
      if (AuthContextStore.session) {
        ...
        const { data: users, error } = await supabase.from("users").select("username, firstName, lastName, enrolledCourses, enrolledGyms, weight, height, phoneNumber, bmi, profileImage, enrolledCoursesCount, enrolledGymsCount, age, goals, gender").eq("email", AuthContextStore.session.user.email as string);
